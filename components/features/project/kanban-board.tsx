...
function SortableBoard({ board }: { board: SingleBoardProps }) {
    const { attributes, listeners, setNodeRef, transform, transition } =
        useSortable({
            id: board.name,
            data: { // Explicitly set data for the draggable board item
                type: "board",
                boardId: board.name // You can add any other relevant board data here
            }
        });
...
export function KanbanBoardWithDndKit() {
    ...
    function handleDragEnd(event: DragEndEvent) {
        ...
        if (activeData?.type === "board" && overData?.type === "board" && activeId !== overId) {
            ...
            return; // Ensure we return after handling board reordering
        }
